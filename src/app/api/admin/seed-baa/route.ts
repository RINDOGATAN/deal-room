...
/**
 * One-shot admin route — seeds the `baa-negotiator` premium skill onto the
 * HOSTED (Vercel/Neon) Deal Room database.
 *
 * WHY THIS EXISTS
 * ----------------
 ...
 * so we let the deployed function perform the exact same per-skill upserts.
 *
 * WHAT IT DOES
 * ------------
 ...
 *   - ContractTemplate (upsert by contractType, incl. category from metadata)
 ...
 *
 * It NEVER deletes anything. Every write is an idempotent upsert, so calling
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      created: !preExisting,
      updated: !!preExisting,
      skillId: premium.skillId,
      version: premium.version,
      isPremium: premium.isPremium,
      price: {
        amount: premium.priceAmount,
        currency: premium.priceCurrency,
