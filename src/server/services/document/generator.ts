...
import { DERIVED_TEXTS, pick } from "./derivedTexts";
...
export async function generateContractData(
  ...
  if (dcParam) {
    ...
    variables.dataCategoriesList = labels.length
      ...
      : pick(DERIVED_TEXTS.dataCategoriesFallback, language);
  }
  ...
  if (establishment) {
    const isES = language === "es";
    variables.processorEstablishmentDisplay = pick(
      DERIVED_TEXTS.establishmentDisplay[establishment] ??
        DERIVED_TEXTS.establishmentDisplay.OTHER,
      language
    );

    const dpfCertified = (dealParams["processor-dpf-certified"] || "") === "yes";
    variables.dpfStatement = pick(
      dpfCertified
        ? DERIVED_TEXTS.dpfStatement.certified
        : DERIVED_TEXTS.dpfStatement.notCertified,
      language
    );
    ...
    if (sgParam) {
      ...
      if (govCommitments) {
        sgLabels.push(pick(DERIVED_TEXTS.govAccessDerivedMeasure, language));
      }
      const letters = "abcdefghijklmnopqrstuvwxyz";
      variables.tiaSafeguardsList = sgLabels.length
        ? sgLabels.map((l, i) => `(${letters[i] || i + 1}) ${l};`).join("\n")
        : pick(DERIVED_TEXTS.safeguardsEmpty, language);
    }
    ...
    if (sgKeys.includes("tech-eu-residency") && variables.processingPurpose) {
      variables.processingPurpose += pick(DERIVED_TEXTS.euResidencyNote, language);
    }
    const hasTechnicalMeasure = sgKeys.some((k) => k.startsWith("tech-"));
    variables.tiaConclusion = pick(
      hasTechnicalMeasure
        ? DERIVED_TEXTS.tiaConclusion.withTechnicalMeasure
        : DERIVED_TEXTS.tiaConclusion.residualRisk,
      language
    );
