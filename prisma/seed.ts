...
async function main() {
  ...
  // ── Self-host: surface premium skills as marketplace stubs ──
  // Hosted seeds real premium content from SKILLS_DIR. A self-hosted build ships
  // none (only the built-in free skills), so the ~28 premium contract skills are
  // invisible locally. Seed metadata-only stubs from the bundled storefront
